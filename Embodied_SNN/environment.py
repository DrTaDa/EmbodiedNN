...
class Environment:
    ...
    def draw_spike_trains(self, agent, canvas_spike_trains, spike_trains, id_min_sender):
        offset_time = self.world_size - 100
        if spike_trains is not None and len(spike_trains['senders']) > 0:
            if id_min_sender > numpy.min(spike_trains['senders']):
                id_min_sender = numpy.min(spike_trains['senders'])
            time_first_spike = numpy.min(spike_trains['times'])
            for sender, time in zip(spike_trains['senders'], spike_trains['times']):
                if sender in agent.brain.output_neurons_id:
                    color = (0, 0, 255)
                else:
                    color = (255, 255, 255)
                time = offset_time + time - time_first_spike
                sender_pos = 3 + ((sender - id_min_sender) * 6)
                cv2.line(canvas_spike_trains, (int(time), sender_pos - 3),
                         (int(time), sender_pos + 3), color, 1)

        for j in range(0, self.world_size - 100, 100):
            canvas_spike_trains[:, j:j + 100, :] = canvas_spike_trains[:, j + 100:j + 200,
                                                   :]

    def draw_and_display(self, entities, agent, score, canvas_spike_trains, spike_trains, id_min_sender):
        ...
        self.draw_spike_trains(agent, canvas_spike_trains, spike_trains, id_min_sender)
        canvas = numpy.concatenate((canvas, canvas_spike_trains), axis=0)

        canvas_spike_trains[:, self.world_size - 100:, :] = (0, 0, 0)
    ...
    def test_agent(self, agent):
        ...
        spike_trains = None
        id_min_sender = 100000
        canvas_spike_trains = numpy.full((300, self.world_size, 3), (0, 0, 0), numpy.uint8)
        ...
        while agent.alive:
            ...
            self.draw_and_display(entities, agent, score, canvas_spike_trains, spike_trains, id_min_sender)

            # Build the observation vector for each agent and send them to the brains
            agent.receive_inputs()

            # Run all the brains for 100 ms
            nest.Simulate(100)

            # Actions
            target, spike_trains = agent.do_something(walls)
